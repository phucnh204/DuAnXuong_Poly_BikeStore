...
<body>
	<div class="container mt-5">
		...
		<div class="tab-content" id="myTabContent">
				...
				<form:form id="productForm" modelAttribute="photoProduct"
					method="post" action="/admin/product/addphoto/insert"
						...
						<div class="col-md-6">
							<div class="form-group">
								<label for="nameBike">Tên xe</label> <input type="text"
									id="namepd" name="nameBike" class="form-control"
			...
			<div
				...
				<div class="table-responsive">
					<table id="productTable" class="table table-striped">
						...
						<tbody>
							<c:forEach var="item" items="${photoProducts}">
								<tr>
									<td>${item.id}</td>
									<td>${item.product.name}</td>
									<td>${item.image}</td>
									<td><a href="/admin/product/photo/delete/${item.id}"
								...
								<ul class="pagination justify-content-center mt-3">
									...
									<c:if test="${currentPage > 0}">
										<li class="page-item"><a class="page-link"
											href="/admin/product/addphoto/${product.id}?pageNo=${currentPage - 1}&activeTab=product-list">Previous</a></li>
									</c:if>
									<c:forEach var="page" items="${pageNumbers}">
										<c:choose>
											...
											<c:otherwise>
												<li class="page-item"><a class="page-link"
													href="/admin/product/addphoto/${product.id}?pageNo=${page}&activeTab=product-list">${page + 1}</a></li>
											</c:otherwise>
										</c:choose>
									</c:forEach>
									<c:if test="${currentPage < totalPages - 1}">
										<li class="page-item"><a class="page-link"
											href="/admin/product/addphoto/${product.id}?pageNo=${currentPage + 1}&activeTab=product-list">Next</a></li>
									</c:if>
								</ul>
							</nav>
						</div>
